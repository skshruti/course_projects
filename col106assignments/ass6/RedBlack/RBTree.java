...
public class RBTree<T extends Comparable, E> implements RBTreeInterface<T, E>  {
 ...
 public RBTree(){
  ...
  //  	if(value==null) System.out.println("wtf");
    	if(root.key==null) {
    		root = new RedBlackNode<T,E>(key, value);
    		//System.out.println(root.getValues().size());
    		root.color = "black";
    		size++;
    		return;
    	}
    	else {
        	RedBlackNode<T, E> papa = null;
        	RedBlackNode<T, E> p = root;
    		while(p.left!=null) {
    			if(p.key.compareTo(key) == 0) {
    				int k=0;
    				while(k<p.getValues().size()) {
    					if(((Comparable) p.getValues().get(k++)).compareTo(value)==0) return;
    				}
    				p.vlist.add(value);
    				return;
    			}
        	...
        	//System.out.println(p.getValues().size());
        	size++;
        	p.parent = papa;
